...
import psycopg2
from psycopg2_pool._compat import Counter, TransactionStatus
from psycopg2_pool.base import Connection

import psycopg2_pool as pool


def test_package_version(mypy):
    cp = mypy.run_on_source(
        """\
from psycopg2_pool import __version__
assert __version__
"""
    )
...
@pytest.mark.skip("BACKPORT: kwargs is not very useful in psycopg2")
def test_kwargs(dsn):
...
def test_configure_badstate(dsn, caplog):
    caplog.set_level(logging.WARNING, logger="psycopg2_pool")
...
def test_intrans_rollback(dsn, caplog):
    ...
    # BACKPORT: Repr does not include the transaction status.
    # assert "INTRANS" in caplog.records[0].message
...
@pytest.mark.skip("BACKPORT: don't know how to backport")
@pytest.mark.crdb_skip("backend pid")
...
def test_fail_rollback_close(dsn, caplog, monkeypatch):
    ...
    assert any(
        ...
        for error in ("DatabaseError", "OperationalError", "OSError")
    )
    # BACKPORT: Repr does not include the transaction status.
...
def test_reopen(dsn):
    ...
    with pytest.raises(psycopg2.OperationalError, match="cannot be reused"):
        p.open()
...
def test_shrink(dsn, monkeypatch):
    ...
    from psycopg2_pool.pool import ShrinkPool
...
def delay_connection(monkeypatch, sec):
    ...
    monkeypatch.setattr(psycopg2, "connect", connect_delay)
