...
# Backport note: psycopg2.connection does not support extra attributes.
# In order to accomodate `_pool` and `_expire_at`, need a sub-class.
#
# Additionally, backport the `conn.execute()` method since the tests
# extensively use this method.
class Connection(psycopg2.extensions.connection, Generic[Row]):
    __slots__ = ("_pool", "_expire_at", "__dict__")

    _pool: Optional["BasePool[Any]"]
    _expire_at: float
...
ConnectionType = TypeVar("ConnectionType", bound=Connection[Any])
