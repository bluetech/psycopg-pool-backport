...
from .base import Connection, ConnectionAttempt, BasePool
from .sched import Scheduler
from .errors import PoolClosed, PoolTimeout, TooManyRequests
from ._compat import Deque, TransactionStatus

logger = logging.getLogger("psycopg2_pool")
...
class ConnectionPool(BasePool[Connection[Any]]):
    ...
    def connection(self, timeout: Optional[float] = None) -> Iterator[Connection[Any]]:
        ...
        *timeout* or `self.timeout` seconds and throw `PoolTimeout` if a
        ...
        t0 = monotonic()
        try:
            # BACKPORT: Ignore "connection already closed" error on exit.
            #           In psycopg3 it's ignored.
            conn.__enter__()
            try:
                yield conn
            finally:
                ...
                except psycopg2.InterfaceError as ei:
                    if "already closed" in str(ei):
                        pass
        finally:
            t1 = monotonic()
    ...
    def _return_connection(self, conn: Connection[Any]) -> None:
        ...
        if conn.info.transaction_status == TransactionStatus.UNKNOWN:
